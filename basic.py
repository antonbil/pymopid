...
class PopList:
    def __init__(self, maxtracks, getList,onpopup,processItem,onLongPress=None):
        ...
        for i in range( maxtracks):
                btn1 = MeasureButtonOnTouch(text="", id=str(i), size_hint_y=None, text_size = (400, 40), valign='middle', halign='left', size=(400, 45), background_color=random.choice(colors))
...
class ListArtist(GridLayout):
    def __init__(self, **kwargs):
        ...
        self.popup = Popup(title="Search artist",content=self,size=(340, 400),size_hint=(None, None),pos_hint={'right': .5, 'top': 1})
...
class LoginScreen(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.popupTracks=PopList(maxtracks, self.getTracks,self.onSelectTrackAction, self.processItem, self.onLongpressTrack)
        self.popupPlaylists=PopList(80, self.getPlaylists,self.onSelectPlaylistAction, self.processPlaylist, self.onLongpressPlaylist)
        self.popupSearch=PopList(maxtracks, self.getSearch,self.onSelectSearchAction, self.processSearch, self.onLongpressSearch)
    ...
    def display_tracks_tree(self,instance=None):
        try:
            ...
            self.popupPlaylists=Popup(title="",separator_height=0,content=tw, size_hint=(None, None), size=(600, Window.height))
            self.pop_up.dismiss()
    ...
    def playlistTreeCallback(self,uri,text=None):
        ...
        buttons=[["Add",lambda x: self.music_controller.playlist_add_mopidy(uri)]
            ...
            self.music_controller.select_and_play_mopidy(song_pos)
                 ...
                 ["Reread playlists",lambda x: [None, self.popupPlaylists.dismiss(),self.popupPlaylistsToNone(), self.display_tracks_tree()
                                              ][0]
                 ]
            ]
        instance=None
    ...
    def onLongpressTrack(self,instance,nr):
        buttons=[["Remove track",self.remove_track],["Remove top",self.remove_top],["Remove bottom",self.remove_bottom]]
