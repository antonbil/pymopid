    ...
    def populate_tree_view(self,tree_view, parent, node):
        if parent is None:
            tree_node = tree_view.add_node(TreeViewLabel(text=node['node_id'], on_touch_up=self.touch1,
                                                        is_open=True,font_size=25,))#,bcolor=random.choice(self.colors)
...
class music_controller:
       ...
       if self.engine=="mpd":
            ...
            for item in list:
                try:
                    ...
                    text='{t:02d}-{artist}-{title}({f:02d}:{s:02d})'.format(f=m,s=s,t=t,artist=item["artist"],title=item["title"])
            ...
            for item in response:
                ...
                #track_no
                t=item["track_no"]
                text='{t:02d}-{artist}-{title}({f:02d}:{s:02d})'.format(f=m,s=s,t=t,artist=item["artists"][0]["name"].encode('utf-8'),title=item["name"].encode('utf-8'))
                playlist.append(text)
            return playlist
    ...
    def do_mopidy_search(self,artist):
            ...
            data = {"jsonrpc": "2.0", "id": 1,"params": {    'query':{'artist': [artist]}  },"exact":"True", "method": "core.library.search"}
            headers = {'content-type': 'application/json'}
            url = 'http://'+mpdServerUrl+':6680/mopidy/rpc'
            #url = 'http://192.168.2.74:6680/mopidy/rpc'
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    totaltime=0
                status={'status' : 'play', 'elapsed' : seconds, 'totaltime' : totaltime,'track' : track,'totaltracks' : 0, 'title': name, 'artist': artist, 'file': uri, 'album':album, 'mode':'mopidy'}
                self.engine="mopidy"
                if 0<s%10<3:
...
class SelectMpdAlbum:
    ...
    def __init__(self,music_controller,colors,popupSearch):
            ...
            grid = BoxLayout(orientation='vertical', size=(450, Window.height-20))#(cols=1, spacing=10, size_hint_y=None, size=(400, Window.height))
            ...
            self.popup = Popup(title="",separator_height=0,content=grid,size_hint=(None, None), size=(450, Window.height))
            i=0
            self.buttons=[]
            #self.dummies=[]
            for i in range(maxalbums):
                    btn1 = MeasureButtonOnTouch(text="", id=str(i), size_hint_y=None, valign='middle', text_size = (400, 40), halign='left', size=(400, 45), background_color=random.choice(self.colors))
            ...
            self.close_button=Button(text="..", id="a", size_hint_y=None, size=(400, 45),on_press=self.goback,background_color=random.choice(self.colors))
    ...
    def dummybutton(self, prevbutton=None):
        btn1 = Button(text="+", id="a", size_hint_y=None, text_size = (40, 40), halign='center', valign='middle', size_hint_x=None, size=(45, 45), background_color=random.choice(self.colors))
